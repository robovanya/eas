 ...
 * @author rvanya
 */
public class DbDrv_Sybase implements IEOC_DB_driver {
    ...
    public boolean SQL_callSqlStatement(Connection cnn, String stm, boolean bCommit) {
        try {
//           System.out.println("SSSQL_callSqlStatement:" + stm); 
...
//QQQ17        cQry = "SELECT * FROM sXyscolumn WHERE table_id = " + tbl_id + " and column_name = '" + col + "'";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                */
                colInf.nullallowed  = !rs.getString("nulls").equals("Y");
                colInf.length       = Integer.parseInt(rs.getString("width"));
                colInf.decimals     = Integer.parseInt(rs.getString("scale"));
                colInf.defaultValue = rs.getString("default_val");
                colInf.realdatatype = rs.getString("domain_name");
                colInf.genericdatatype = getGenericDataType(colInf.realdatatype);
                colInf.comment      = rs.getString("col_remark");
...
//QQQ17              case "comment": { cProperty = "remarks"; break; }     // remarks
              case "datatype": { cProperty = "domain_name"; break; } 
              case "nullallowed": { cProperty = "nulls"; break; } // nulls Y/N
              case "length": { cProperty = "width"; break; } // width ?
              case "decimals": { cProperty = "scale"; break; }  // scale ?
              case "comment": { cProperty = "col_remark"; break; }     // remarks
              default: {
